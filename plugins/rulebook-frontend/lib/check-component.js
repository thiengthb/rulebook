 ...
 */
import { COMPOSITOR_SAFE_ANIMATED_PROPS, FOREIGN_ICON_PACKAGES, FOREIGN_TOAST_PACKAGES, RULE_BY_ID, } from '../rules/frontend.rules.js';
...
function jsxText(region) {
    ...
    for (let i = 0; i < region.length; i++) {
        ...
        if (c === '{') {
            ...
            depth--;
            if (depth < 0)
                return null; // a closer with no opener — this region began inside code
            out[i] = ' ';
        }
        else if (depth > 0 && c !== '\n') {
            out[i] = ' ';
...
const MOTION_PROP_BLOCK = /\b(?:animate|initial|exit|whileHover|whileTap|whileInView)\s*=\s*\{\{([^}]*)\}\}/g;
/**
...
export function checkComponent(source, opts = {}) {
    ...
    if (applies('emoji-as-icon')) {
        ...
        while ((m = JSX_TEXT.exec(code))) {
            ...
            if (reported.has(lineIdx))
                continue;
            reported.add(lineIdx);
            push('emoji-as-icon', lineIdx, 'An emoji is rendered as a UI marker in this element.', 'Replace it with a `lucide-react` icon component. Emoji is allowed only inside text a model emits verbatim.');
    ...
    lines.forEach((line, idx) => {
        ...
        if (applies('icon-set')) {
            for (const pkg of FOREIGN_ICON_PACKAGES) {
                if (new RegExp(`from\\s*['"]${pkg.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`).test(line)) {
                    push('icon-set', idx, `Icons are imported from \`${pkg}\`, which is not this platform's icon set.`, 'Import the equivalent icon from `lucide-react` instead, and drop this dependency.');
                }
            }
            if (/<svg[\s>]/.test(line) &&
                ...
                push('icon-set', idx, 'A hand-written `<svg>` is used where an icon component belongs.', 'Use a `lucide-react` icon. Inline SVG is only for rendering DATA (a gauge, a sparkline, a score ring).');
            }
        }
        ...
        if (applies('hardcoded-color') && !inThemeBlock[idx]) {
            ...
            const stillHasLiteral = HEX_COLOR.test(withoutVarFallbacks) || FN_COLOR.test(withoutVarFallbacks);
            if ((hasHex || hasFn) && stillHasLiteral && !looksLikeUrlFragment && !isCssVarDefinition) {
                push('hardcoded-color', idx, 'A literal color value is written into this line, so it cannot follow the light/dark theme.', 'Use a theme token (a CSS variable / a Tailwind semantic class). Literal colors belong only in the `@theme` block.');
            }
        }
        /* ── forward-ref: React 19 passes ref as a normal prop ──────────────────────────── */
        if (applies('forward-ref') && /\bforwardRef\s*[<(]/.test(line)) {
            push('forward-ref', idx, '`forwardRef` is used, which this stack no longer needs.', 'Take `ref` as an ordinary prop on the component and delete the wrapper.');
        }
        /* ── dangerous-html ─────────────────────────────────────────────────────────────── */
        if (applies('dangerous-html') && /dangerouslySetInnerHTML/.test(line)) {
            ...
            if (!sanitized) {
                push('dangerous-html', idx, 'Raw HTML is injected here with no visible sanitizer on the same expression.', 'Render as text, or pass the value through a sanitizer before it reaches this prop.');
            }
        }
        /* ── toast-library ──────────────────────────────────────────────────────────────── */
        if (applies('toast-library')) {
            for (const pkg of FOREIGN_TOAST_PACKAGES) {
                if (new RegExp(`from\\s*['"]${pkg.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}['"]`).test(line)) {
                    push('toast-library', idx, `Toasts come from \`${pkg}\`, which is not the one this platform standardised on.`, 'Import `toast` from `sonner` and render `<Toaster />` once in the app shell.');
                }
            }
        }
        ...
        if (applies('client-secret') && isClientFile) {
            ...
            if (m && !/^(?:NEXT_PUBLIC_|VITE_|NODE_ENV$)/.test(m[1])) {
                push('client-secret', idx, `\`${m[1]}\` is read in a client component, so its value ships in the browser bundle.`, 'Read it in a server component / route handler and pass down only the derived value — or prefix it `NEXT_PUBLIC_` if it is genuinely public.');
            }
        }
        /* ── debug-logging ──────────────────────────────────────────────────────────────── */
        if (applies('debug-logging') && /\bconsole\.log\s*\(/.test(line)) {
            push('debug-logging', idx, 'A `console.log` is left in this line.', 'Delete it. Deliberate server-side logging should use a real logger, not `console.log`.');
        }
    });
    ...
    if (applies('animated-property') && IMPORTS_MOTION.test(code)) {
        ...
        while ((m = MOTION_PROP_BLOCK.exec(code))) {
            ...
            if (bad.length) {
                push('animated-property', lineIdx, `This animation drives ${bad.map((b) => `\`${b}\``).join(', ')}, which forces layout or paint on every frame.`, 'Animate `transform` (x/y/scale/rotate) and `opacity` only; achieve the rest by animating a transform on a wrapper.');
            }
        }
    }
