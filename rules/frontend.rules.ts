...
export type Rule = {
  /** Stable id — the client sees this, so it must not encode rule text. */
  id: string;
  /** Which file kinds this applies to. */
  applies: ReadonlyArray<'tsx' | 'jsx' | 'ts' | 'css'>;
  severity: Severity;
  /** One-line label for grouping in the report. Not the rule text. */
  label: string;
};
...
export type RuleId = (typeof FRONTEND_RULES)[number]['id'];

export const RULE_BY_ID: ReadonlyMap<RuleId, Rule> = new Map(
  FRONTEND_RULES.map((r) => [r.id, r as Rule]),
);
